...
const Pagination = () =>
{
   ...
   return (
      <div className="flex bg-white rounded-lg font-[Poppins] py-6 justify-center">
         <button onClick={back} className="h-[45px] border-2 border-r-0 border-indigo-600
         ...
         {
            pages.map((pg, i) => (
               <button key={i} onClick={() => setCur(pg.page)} className={`h-[45px] border-2 border-r-0 border-indigo-600
               w-12 ${cur === pg.page && 'bg-indigo-600 text-white'}`}>{pg.page}</button>
            ))
         }
         <button onClick={Next} className="h-[45px] border-2  border-indigo-600
